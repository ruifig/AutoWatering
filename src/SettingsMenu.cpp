...
namespace
{
	...
	{ \
		{LayoutHelper::getMenuButtonPos(COL, LINE).x, LayoutHelper::getMenuButtonPos(COL,LINE).y + (SUBLINE * (AW_TOUCHUI_GRAPH_HEIGHT/3)), 32, AW_TOUCHUI_GRAPH_HEIGHT/3}, \
	...
	{ \
		{LayoutHelper::getMenuButtonPos(1+COL, 1).x, LayoutHelper::getMenuButtonPos(1+COL,1).y + (CELL_LINE * (AW_TOUCHUI_GRAPH_HEIGHT/2)), 32, AW_TOUCHUI_GRAPH_HEIGHT/2}, \
		HAlign::Center, VAlign::Center, \
		TINY_FONT, \
		AW_TOUCHUI_GRAPH_VALUES_TEXT_COLOUR, AW_TOUCHUI_GRAPH_VALUES_BKG_COLOUR, \
		WidgetFlag::EraseBkg | FLAGS \
...
void SettingsMenu::init()
{
	...
	// First line
	initButton(ButtonId::CloseAndSave, LayoutHelper::getMenuButtonPos(0,0), AW_TOUCHUI_SCREEN_BKG_COLOUR, img_Save);
	initButton(ButtonId::Calibrate, LayoutHelper::getMenuButtonPos(CALIBRATE_BTN_COL,0), AW_TOUCHUI_SCREEN_BKG_COLOUR, img_Ruler);
	initButton(ButtonId::SensorInterval, LayoutHelper::getMenuButtonPos(SENSORINTERVAL_BTN_COL,0), AW_TOUCHUI_SCREEN_BKG_COLOUR, img_SetSensorInterval);
	initButton(ButtonId::ShotDuration, LayoutHelper::getMenuButtonPos(SHOTDURATION_BTN_COL,0), AW_TOUCHUI_SCREEN_BKG_COLOUR, img_SetWateringDuration);
	// Leaving one empty grid space intentionally, so the CloseAndIgnore button is spaced away from the others
	initButton(ButtonId::CloseAndIgnore, LayoutHelper::getMenuButtonPos(5,0), AW_TOUCHUI_SCREEN_BKG_COLOUR, img_Close);

	// Second line
	initButton(ButtonId::ResetRange, LayoutHelper::getMenuButtonPos(CALIBRATE_BTN_COL-1,1), AW_TOUCHUI_SCREEN_BKG_COLOUR, img_Eraser).setClearWhenHidden(false);
	initButton(ButtonId::SetThreshold, LayoutHelper::getMenuButtonPos(CALIBRATE_BTN_COL+1,1), AW_TOUCHUI_SCREEN_BKG_COLOUR, img_SetThreshold).setClearWhenHidden(false);
	initButton(ButtonId::Minus, LayoutHelper::getMenuButtonPos(0,1), AW_TOUCHUI_SCREEN_BKG_COLOUR, img_Remove).setClearWhenHidden(false);
	initButton(ButtonId::Plus, LayoutHelper::getMenuButtonPos(0,1), AW_TOUCHUI_SCREEN_BKG_COLOUR, img_Add).setClearWhenHidden(false);
...
void SettingsMenu::draw()
{
	...
	if (m_clearMenuSecondLine)
	{
		...
		fillRect(LayoutHelper::getMenuLineArea(1), AW_TOUCHUI_SCREEN_BKG_COLOUR);
	}
