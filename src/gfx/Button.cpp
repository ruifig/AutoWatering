...
void BaseButton::move(int16_t x, int16_t y, bool eraseCurrentPosition)
{
	if (eraseCurrentPosition)
	{
		if (m_enabled)
		{
			fillRect(m_pos, AW_TOUCHUI_SCREEN_BKG_COLOUR );
		}
	}
...
void ImageButton::draw(bool forceDraw)
{
	...
	{
		drawRGBBitmap_P(m_pos.x, m_pos.y, m_img.bmp, m_img.mask, m_img.width, m_img.height, AW_TOUCHUI_SCREEN_BKG_COLOUR);
	}
	else
	{
		drawRGBBitmapDisabled_P(m_pos.x, m_pos.y, m_img.bmp, m_img.mask, m_img.width, m_img.height, AW_TOUCHUI_SCREEN_BKG_COLOUR);
	}
