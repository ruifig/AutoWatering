...
{
	{
		{10, 10, AW_TOUCHUI_SCREEN_WIDTH - 20, AW_TOUCHUI_SCREEN_HEIGHT - 20 }, // box
		HAlign::Left, VAlign::Top,
		LARGE_FONT,
		AW_TOUCHUI_INTRO_TEXT_COLOUR,
		AW_TOUCHUI_SCREEN_BKG_COLOUR,
		WidgetFlag::None
	},
...
{
	if (m_outer.m_timeInState >= AW_TOUCHUI_INTRO_DURATION)
	{
...
namespace { namespace BootMenu {
	constexpr Rect getResetButtonPos()
	{
		...
		return { {(AW_TOUCHUI_SCREEN_WIDTH-width)/2, (AW_TOUCHUI_SCREEN_HEIGHT-height)/2}, width, height};
	}
	constexpr Rect getConfirmButtonPos()
	{
		constexpr int width = 200;
		constexpr int height = 40;
		return { {(AW_TOUCHUI_SCREEN_WIDTH-width)/2, 10}, width, height};
	}
...
void GraphicalUI::BootMenuState::tick(float deltaSeconds)
{
	...
	if (m_waitingResetConfirmation == false)
	{
		...
		printAligned(
			{{0, BootMenu::getResetButtonPos().bottom() + 10}, AW_TOUCHUI_SCREEN_WIDTH, 40},
			HAlign::Center, VAlign::Center,
			F(" Loading saved config in... ")
		);
		printAligned(
			{{0, BootMenu::getResetButtonPos().bottom() + 40}, AW_TOUCHUI_SCREEN_WIDTH, 40},
			HAlign::Center, VAlign::Center,
		...
		else if (m_outer.m_touch.pressed && BootMenu::getResetButtonPos().contains(m_outer.m_touch.pos))
		{
			...
			TFTeSPIWrapper::getInstance()->setTextColor(Colour_Yellow, Colour_Black);
			printAligned(
				{{0, BootMenu::getConfirmButtonPos().bottom() + 5}, AW_TOUCHUI_SCREEN_WIDTH, 40},
				HAlign::Center, VAlign::Center,
				F("Click confirm if you are really sure.")
			);
			printAligned(
				{{0, BootMenu::getConfirmButtonPos().bottom() + 25}, AW_TOUCHUI_SCREEN_WIDTH, 40},
				HAlign::Center, VAlign::Center,
...
void GraphicalUI::BootMenuState::onEnter()
{
	TFTeSPIWrapper::getInstance()->setTextColor(Colour_Yellow, Colour_Black);
	printAligned(
		{{0, BootMenu::getResetButtonPos().top() - 40}, AW_TOUCHUI_SCREEN_WIDTH, 40},
		HAlign::Center, VAlign::Center,
...
{ \
	{ \
		...
		LayoutHelper::getHistoryPlotRect(SENSOR_INDEX).y + (LINE_INDEX * (AW_TOUCHUI_GRAPH_HEIGHT/3)),  \
		AW_TOUCHUI_SCREEN_WIDTH - (LayoutHelper::getHistoryPlotRect(SENSOR_INDEX).x + LayoutHelper::getHistoryPlotRect(SENSOR_INDEX).width + 2), \
		AW_TOUCHUI_GRAPH_HEIGHT/3, \
	}, \
	HAlign::Center, VAlign::Center, \
	TINY_FONT, \
	AW_TOUCHUI_GRAPH_VALUES_TEXT_COLOUR, AW_TOUCHUI_GRAPH_VALUES_BKG_COLOUR, \
...
NumLabel<true> sensorLabels[AW_TOUCHUI_VISIBLE_NUM_PAIRS][3] =
{
	{
		{ sensor0line0 },
		{ sensor0line1 },
...
#if AW_TOUCHUI_VISIBLE_NUM_PAIRS>1
	,
	{
		{ sensor1line0 },
		{ sensor1line1 },
...
#endif
#if AW_TOUCHUI_VISIBLE_NUM_PAIRS>2
	,
	{
		{ sensor2line0 },
		{ sensor2line1 },
...
#endif
#if AW_TOUCHUI_VISIBLE_NUM_PAIRS>3
	,
	{
		{ sensor3line0 },
		{ sensor3line1 },
...
{ \
	...
	HAlign::Center, VAlign::Center, \
	SMALL_FONT, \
	AW_TOUCHUI_HUMIDITY_LABEL_TEXT_COLOUR, AW_TOUCHUI_GRAPH_VALUES_BKG_COLOUR, \
...
{ \
	...
	HAlign::Center, VAlign::Center, \
	SMALL_FONT, \
	AW_TOUCHUI_TEMPERATURE_LABEL_TEXT_COLOUR, AW_TOUCHUI_GRAPH_VALUES_BKG_COLOUR, \
	WidgetFlag::EraseBkg | WidgetFlag::DrawBorder\
};
...
{ \
	...
	HAlign::Center, VAlign::Center, \
	SMALL_FONT, \
	AW_TOUCHUI_RUNNINGTIME_LABEL_TEXT_COLOUR, AW_TOUCHUI_GRAPH_VALUES_BKG_COLOUR, \
	WidgetFlag::EraseBkg | WidgetFlag::DrawBorder\
};
...
{ \
	...
	HAlign::Center, VAlign::Center, \
	SMALL_FONT, \
	AW_TOUCHUI_BATTERYLEVEL_LABEL_TEXT_COLOUR, AW_TOUCHUI_GRAPH_VALUES_BKG_COLOUR, \
	WidgetFlag::EraseBkg | WidgetFlag::DrawBorder \
};
...
GraphicalUI::OverviewState::Group* GraphicalUI::OverviewState::tryGetGroupByPairIndex(int8_t pairIndex)
{
	...
	if (screenSlot>=0 && screenSlot<AW_TOUCHUI_VISIBLE_NUM_PAIRS)
	{
...
{
	TFTeSPIWrapper::getInstance()->setTextColor(AW_TOUCHUI_GRAPH_VALUES_TEXT_COLOUR, Colour_VeryDarkGrey);
	TFTeSPIWrapper::getInstance()->setFont(TINY_FONT);
	for(int i=0; i<AW_TOUCHUI_VISIBLE_NUM_PAIRS; i++)
	{
		int pairIndex = i + m_topSlotPairIndex;
		if (pairIndex < AW_MAX_NUM_PAIRS)
		{
			printAligned(
				{{0, LayoutHelper::getHistoryPlotRect(i).top()}, AW_TOUCHUI_GROUP_NUM_WIDTH, LayoutHelper::getHistoryPlotRect(i).height},
				HAlign::Center, VAlign::Center,
...
{
	m_topSlotPairIndex = clamp(m_topSlotPairIndex+inc, 0, AW_MAX_NUM_PAIRS - AW_TOUCHUI_VISIBLE_NUM_PAIRS);
	...
	for(int idx = 0; idx<AW_TOUCHUI_VISIBLE_NUM_PAIRS; idx++)
	{
...
void GraphicalUI::updateTouch(float deltaSeconds)
{
	...
	if (touchState == TouchState::Released)
	{
		...
		if (m_touch.sleeping)
		{
			...
			TFTeSPIWrapper::getInstance()->setBacklightBrightness(AW_TOUCHUI_SCREEN_DEFAULT_BRIGHTNESS);
		}
		else
		{
			m_touch.pos = {p.x, p.y};
	...
	else
	{
		if (m_touch.sleeping)
		{
			...
			{
				m_touch.currentBrightness -= deltaSeconds * AW_TOUCHUI_SCREEN_OFF_DIM_SPEED;
		...
		else
		{
			...
			if ((AW_TOUCHUI_SCREEN_OFF_TIMEOUT!=0) && (m_touch.secondsSinceLastTouch > AW_TOUCHUI_SCREEN_OFF_TIMEOUT))
			{
				turnOffLED();
			}
		}
...
void GraphicalUI::turnOffLED()
{
	...
	m_touch.currentBrightness = AW_TOUCHUI_SCREEN_DEFAULT_BRIGHTNESS;
}
...
void GraphicalUI::changeToState(DisplayState& newState)
{
	...
	TFTeSPIWrapper::getInstance()->fillScreen(AW_TOUCHUI_SCREEN_BKG_COLOUR);
	m_state = &newState;
    m_timeInState = 0.0f;
	m_state->onEnter();
}
