#if PORTING_TO_RP2040

#include "Mux16Channels.h"

namespace cz
{

namespace
{
	const uint8_t muxChannel[16][4] = {
		{0, 0, 0, 0},  // channel 0
		{1, 0, 0, 0},  // channel 1
		{0, 1, 0, 0},  // channel 2
		{1, 1, 0, 0},  // channel 3
		{0, 0, 1, 0},  // channel 4
		{1, 0, 1, 0},  // channel 5
		{0, 1, 1, 0},  // channel 6
		{1, 1, 1, 0},  // channel 7
		{0, 0, 0, 1},  // channel 8
		{1, 0, 0, 1},  // channel 9
		{0, 1, 0, 1},  // channel 10
		{1, 1, 0, 1},  // channel 11
		{0, 0, 1, 1},  // channel 12
		{1, 0, 1, 1},  // channel 13
		{0, 1, 1, 1},  // channel 14
		{1, 1, 1, 1}   // channel 15
	};
}

Mux16Channels::Mux16Channels(MCP23017WrapperInterface& ioExpander, IOExpanderPin s0, IOExpanderPin s1, IOExpanderPin s2,
                             IOExpanderPin s3, ArduinoPin zPin)
    : m_ioExpander(ioExpander)
	, m_sPins{s0, s1, s2, s3}
	, m_zPin(zPin)
{
}

void Mux16Channels::begin()
{
	pinMode(m_zPin.raw, INPUT);

	for (auto&& pin : m_sPins)
	{
		m_ioExpander.pinMode(pin, OUTPUT);
	}
}

int Mux16Channels::read(MultiplexerPin channel) const
{
	// Set s0-s3
	for (uint8_t i = 0; i < 4; i++)
	{
		m_ioExpander.digitalWrite(m_sPins[i], muxChannel[channel.raw][i]);
	}

#if MOCK_COMPONENTS
	return 0;
#else
	return analogRead(m_zPin.raw);
#endif
}

}  // namespace cz

#endif
