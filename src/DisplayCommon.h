...
struct LayoutHelper
{
	static constexpr Rect getHistoryPlotRect(int index)
	{
		Rect rect(
			...
			AW_TOUCHUI_GROUP_NUM_WIDTH+3,
			// 1+ to leave a line for the box
			// + X at the end to add a space between different history plots
			1 + (index*(AW_TOUCHUI_GRAPH_HEIGHT + 5)),
			AW_TOUCHUI_GRAPH_NUMPOINTS, // width
			AW_TOUCHUI_GRAPH_HEIGHT // height
		);
	...
	{
		Rect rect { getMenuButtonPos(0,2), {AW_TOUCHUI_SCREEN_WIDTH, AW_TOUCHUI_SCREEN_HEIGHT} };
		return rect;
	}
...
static_assert((AW_TOUCHUI_SCREEN_HEIGHT - (LayoutHelper::getHistoryPlotRect(3).y + LayoutHelper::getHistoryPlotRect(3).height)) > 64, "Need enough space left at the bottom for the menu (64 pixels high)");
